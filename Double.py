...
class DoubleDQN:
    ...
    def _build_net(self):
        def build_layers(s, c_names, n_l1, w_initializer, b_initializer):
            ...
            return out
        # ------------------ build evaluate_net ------------------
        self.s = tf.keras.Input(tf.float32, [None, self.n_features], name='s')  # input
        self.q_target = tf.keras.Input(tf.float32, [None, self.n_actions], name='Q_target')  # for calculating loss
        ...
        # ------------------ build target_net ------------------
        self.s_ = tf.keras.Input(tf.float32, [None, self.n_features], name='s_')    # input
    ...
    def plot_cost(self,name= 'RL'):
        ...
        np.save('.\\data\\NNcost'+name+'.npy',np.array(self.cost_his))
        plt.ylabel('Cost')
